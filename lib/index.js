...
class Cryo extends SQLite {
  constructor(cryoConfig, appVersion = null, location = null) {
    ...
    this._queries.allHashes = `
      SELECT hash, type
      FROM actions
      WHERE goblin = $goblin
        AND (type = 'persist' OR type = 'revoke')
      ORDER BY rowid DESC
    `;
  ...
  *branch(resp, msg, next) {
  ...
  getLastCommonHash(resp, msg) {
    ...
    for (const {hash, type} of Object.values(hashes)) {
  ...
  getLastHashes(resp, msg) {
    ...
    const hashes = this.stmts(db).allLastHashes.all();
    return hashes;
  }
  ...
  getLastPersist(resp, msg) {
    ...
    const data = this.stmts(db).lastPersistedAction.get({goblin});
    return data;
  }

  getLastRevoke(resp, msg) {
    if (!this.tryToUse(resp)) {
      return;
    }
    ...
    const data = this.stmts(db).lastRevoke.get({goblin});
    return data;
  }
  ...
  *actionsSync(resp, msg, next) {
    ...
    const {db, goblin, desktopId} = msg.data;
    if (!this._open(db, resp)) {
      return;
    }
    ...
    const hashes = this.stmts(db).allHashes.all({goblin});
    ...
    resp.log.dbg(
      `@@@ LAST COMMON HASH: ${data.hash} for ${data.type} / ${goblin}`
    );
    const {hash, type} = data;

    const isRevoked = hashes.length && hashes[0].type === 'revoke';
    if (isRevoked) {
      /* 'revoke' is the last common action, skip because nothing new */
      if (type === 'revoke') {
        return;
      }
  ...
  *actionsSyncDB(resp, msg, next) {
    ...
    const lock = `sync-db-${db}`;
    yield this._syncLock.lock(lock, next);
    try {
      const {data = {}} = yield resp.command.send(
        'cryo.getLastHashes',
        {_xcraftRPC: true, db},
      ...
      for (const {goblin, hash, type} of data) {
        const result = this.stmts(db).rowidByHash.get({hash});
        ...
        yield resp.command.send('cryo.actionsSync', {db, goblin}, next);
      }
    } finally {
      this._syncLock.unlock(lock);
    }
  }
}
