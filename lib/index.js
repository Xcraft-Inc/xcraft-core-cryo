...
class Cryo extends SQLite {
  constructor(cryoConfig, appVersion = null, location = null) {
    ...
    this._queries.freeze = `
      INSERT INTO actions
      VALUES ($timestamp, $goblin, $action, $version, $hash, $type, $source, $uuid, NULL)
    `;
  ...
  *actionsSync2(resp, msg, next) {
    ...
    const result = yield resp.command.send(
      'goblin.ripley',
      {actions: stagedActions, db, desktopId},
      next
    );
    if (!result?.data) {
      return;
  ...
  *actionsSyncDB(resp, msg, next) {
    ...
    try {
      ...
      for (const {goblin, hash} of data) {
        ...
        yield resp.command.send(
          'cryo.actionsSync',
          {desktopId, db, goblin, $skipLock: true},
          next
        );
      }
    } finally {
