...
class Cryo extends SQLite {
  constructor(cryoConfig, appVersion = null, location = null) {
    ...
    this._queries.freeze = `
      INSERT INTO actions
      VALUES ($timestamp, $goblin, $action, $version, $hash, $type, $source, $uuid, $commitId)
    `;
      ...
      () => {
        ...
        if (this._cryoConfig?.enableFTS) {
          this.function(dbName, 'onInsertLastAction', (goblin) =>
            this._sendNotifs(resp, 'onInsert', goblin)
          );

          this.function(dbName, 'onUpdateLastAction', (goblin) =>
            this._sendNotifs(resp, 'onUpdate', goblin)
          );

          this.function(dbName, 'onDeleteLastAction', (goblin) =>
            this._sendNotifs(resp, 'onDelete', goblin)
          );
        }
      },
      /* onMigrate */
  ...
  async immediate(resp, msg) {
    ...
    await this._syncLock.lock(this.getLocation());
    try {
      await this._wait(() => this.stmts(db).immediate.run());
      this._inTransaction = true;
    } catch (ex) {
      await this._syncLock.unlock(this.getLocation());
      throw ex;
    }
  }
  ...
  commit(resp, msg) {
    ...
    this._inTransaction = false;
    this._syncLock.unlock(this.getLocation());

    /* Send all pending events after commit (events coming from the triggers) */
    for (const {topic, goblin} of this._triggerNotifs) {
      resp.events.send(topic, goblin);
    }
    this._triggerNotifs.length = 0;
  }
  ...
  freeze(resp, msg) {
    ...
    try {
      ...
      const commitId = action.meta?.commitId;
  ...
  *branch(resp, msg, next) {
  ...
  ripleyCommit(resp, msg, next) {
    ...
    const commitActionsStmt = `
      ...
      WHERE goblin IN (${goblinIds.map((id) => `'${id}'`).join(',')})
        AND type = 'persist'
      ORDER BY rowid DESC
      LIMIT 1
    `;
  ...
  updateActionsAfterSync(resp, msg) {
    ...
    const {db, serverCommitId, stagedActions} = msg.data;
    if (!this._open(db, resp)) {
      return;
    }
    ...
    const commitActionsStmt = `
      ...
      WHERE rowid IN (${stagedActions.map(({rowid}) => rowid).join(',')})
    `;
