...
class Cryo {
  ...
  _open(dbName) {
    ...
    this._stmts[dbName] = {};
    this._stmts[dbName].create = this._db[dbName]
      .prepare(
        `CREATE TABLE IF NOT EXISTS actions (timestamp TEXT, goblin TEXT, action JSON)`
      )
      .run();
    this._stmts[dbName].index = this._db[dbName]
      .prepare(
    ...
    this._stmts[dbName].select = this._db[dbName].prepare(
      `SELECT * FROM actions WHERE timestamp <= $timestamp GROUP BY goblin HAVING max(timestamp)`
    );
  ...
  thaw(resp, msg) {
    ...
    for (const row of this._stmts[db].select.iterate({timestamp})) {
