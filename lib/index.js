...
class Cryo extends SQLite {
  constructor() {
    ...
    this._queries.partialThaw = `SELECT * FROM actions WHERE goblin LIKE $type AND timestamp <= $timestamp GROUP BY goblin HAVING max(timestamp) LIMIT $length OFFSET $offset`;
    this._queries.partialFrozen = `SELECT count(*) AS count, timestamp FROM (SELECT * FROM actions WHERE goblin LIKE $type AND timestamp <= $timestamp GROUP BY goblin HAVING max(timestamp) ORDER BY timestamp)`;
  ...
  thaw(resp, msg) {
    ...
    if (type && length) {
      stmt = 'partialThaw';
      Object.assign(params, {type, length, offset});
    }
  ...
  *branch(resp, msg, next) {
  ...
  dump(resp, msg) {
    ...
    try {
      ...
      super.exec(
        dbName,
        'CREATE TABLE IF NOT EXISTS dump.actions(timestamp TEXT, goblin TEXT, action JSON, version TEXT)'
      );
      super.exec(
        dbName,
        `INSERT INTO dump.actions SELECT * FROM main.actions WHERE timestamp <= "${timestamp}" GROUP BY goblin HAVING max(timestamp)`
      );
    } finally {
