...
class Cryo {
  ...
  _open(dbName) {
    ...
    const thawSQL = `SELECT * FROM actions WHERE timestamp <= $timestamp GROUP BY goblin HAVING max(timestamp)`;
    this._queries = {
      freeze: `INSERT INTO actions VALUES ($timestamp, $goblin, $action)`,
      thaw: thawSQL,
      frozen: `SELECT count(*) AS count, timestamp FROM (${thawSQL} ORDER BY timestamp)`,
    };
  ...
  thaw(resp, msg) {
    ...
    this._prepare(db, 'thaw'); // FIXME: check if it's really necessary
  }
  ...
  *branch(resp, msg, next) {
    ...
    const dst = Cryo._path(`${db}.${this.timestamp()}`);
    try {
