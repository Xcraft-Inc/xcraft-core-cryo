...
class Cryo extends SQLite {
  constructor(appVersion = null, location = null) {
    if (!location) {
  ...
  _open(dbName, resp) {
    ...
    const res = super.open(
      ...
      () => {
        ...
        super.exec(dbName, 'PRAGMA journal_mode = WAL');
   ...
   * @return {Object} the whole payload.
   */
  freeze(resp, msg) {
    if (!this.tryToUse(resp)) {
      return;
   ...
   * @param {Object} resp - Response object provided by busClient.
   * @param {Object} msg - Original bus message.
   * @return {int} number of sent events
   */
  thaw(resp, msg) {
    if (!this.tryToUse(resp)) {
      return;
  ...
  *branch(resp, msg, next) {
   ...
   * @param {Object} resp - Response object provided by busClient.
   * @param {Object} msg - Original bus message.
   * @return {Array} item Object with type and count.
   */
