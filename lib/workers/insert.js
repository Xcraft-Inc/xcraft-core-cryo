...
async function populate({
  ...
  try {
    ...
    if (enableVEC) {
      noBadEx(() => sqlite.stmts(db).immediate.run());
      noBadEx(() => sqlite.stmts(db).populateVec.run({defaultLocale}));
      noBadEx(() => sqlite.stmts(db).populateVecIndex.run());
      noBadEx(() => sqlite.stmts(db).commit.run());
    }
...
async function embed({
  ...
  try {
    ...
    await SQLite.wait(() => {
      noBadEx(() => sqlite.stmts(db).immediate.run());
      noBadEx(() => sqlite.stmts(db).deleteEmbed.run({documentId}));
      noBadEx(() => sqlite.stmts(db).deleteEmbedIdx.run({documentId}));
      noBadEx(() => sqlite.stmts(db).insertEmbed.run({goblin, defaultLocale}));
      noBadEx(() => sqlite.stmts(db).insertEmbedIdx.run({documentId, goblin}));
      noBadEx(() => sqlite.stmts(db).commit.run());
    });
  } catch (ex) {
    console.error(ex.stack || ex.message || ex);
...
async function refreshEmbeddings({
  ...
  try {
    ...
    await SQLite.wait(() => {
      noBadEx(() => sqlite.stmts(db).immediate.run());
      noBadEx(() => sqlite.stmts(db).deleteOutdatedEmbeds.run());
      noBadEx(() => sqlite.stmts(db).deleteOutdatedEmbedIndex.run());
      noBadEx(() => sqlite.stmts(db).insertOutdatedEmbeds.run({defaultLocale}));
      noBadEx(() => sqlite.stmts(db).insertOutdatedEmbedIndex.run());
      noBadEx(() => sqlite.stmts(db).commit.run());
    });
  } catch (ex) {
    console.error(ex.stack || ex.message || ex);
