...
class SoulSweeper {
  ...
  sweepByCount(count = 4, dryrun = true) {
    ...
    this.#log(dryrun, `sweepByCount count=${count}`);
    ...
    try {
      ...
      this.#log(
        dryrun,
        `${changes} are sweeped out in order to keep ${count} persists by id`
      );
      return changes;
    } finally {
      this.#after(changes);
  ...
  sweepByDatetime(datetime = this.#sqlite.timestamp(), dryrun = true) {
    ...
    this.#log(dryrun, `sweepByDatetime datetime=${datetime}`);

    if (dryrun) {
      ({changes} = this.#sweepByDatetimeDryrun.get({datetime}));
      this.#log(dryrun, `${changes} can be sweeped out before ${datetime}`);
      return changes;
    }
    ...
    try {
      ...
      this.#log(dryrun, `${changes} are sweeped out before ${datetime}`);
      return changes;
    } finally {
      this.#after(changes);
    }
   ...
   * @param {boolean} dryrun if true, reports and nothing is deleted
   */
  sweepForWeeks(days = 30, dryrun = true) {
    const date = new Date();
