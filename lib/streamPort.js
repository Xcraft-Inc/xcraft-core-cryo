...
class WriterTimeoutError extends Error {
  constructor(message) {
    super();
    this.name = 'WriterTimeoutError';
  }
}
...
class MessagePortWritable extends Writable {
  ...
  /* Destroy the stream if the reader seems dead */
  #lastDemand = Date.now();
  #watchdog = null;
  #timeoutDelay;
  ...
  constructor(port, options) {
    ...
    const config = require('xcraft-core-etc')().load('xcraft-core-cryo');
    this.#timeoutDelay = config.workers.streamPort.timeout;
  ...
  #startWatchdog() {
    this.#watchdog = setInterval(() => {
      const elapsed = Date.now() - this.#lastDemand;
      if (elapsed > this.#timeoutDelay && !this.#closed) {
        this.destroy(new WriterTimeoutError('No demand from reader'));
      }
    }, 10000);
