...
const {WritableSQL} = require('./streamSQL.js');
const Streamer = require('xcraft-core-transport/lib/streamer.js');
const SoulSweeper = require('./soulSweeper.js');
const {Piscina} = require('piscina');
const {MessagePortReadable} = require('./streamPort.js');

const versionPragma = 13;

async function unlinkDatabase(dbPath) {
  await xFs.fse.remove(dbPath);
  await xFs.fse.remove(dbPath + '-journal');
  await xFs.fse.remove(dbPath + '-wal');
  await xFs.fse.remove(dbPath + '-shm');
}
...
class Cryo extends SQLite {
  ...
  constructor(cryoConfig, appVersion = null, location = null) {
    ...
    if (enableFTS) {
      this._ftsTables = `
        ...
        BEGIN
          INSERT INTO lastPersistedActions(goblin, action) VALUES (new.goblin, new.action)
          ON CONFLICT
          DO UPDATE SET
            action = new.action;
        END;
    ...
    if (enableVEC && enableFTS) {
      ...
      this._vecTables = `
        ...
        CREATE TABLE IF NOT EXISTS embeddingsIndex (
          ...
          documentRowid INTEGER NOT NULL
        );
    ...
    this._queries.allStagedActions = `
      WITH last_persist AS (
        SELECT max(rowid) as rowid, goblin
        FROM actions
      ...
      WHERE commitId IS NULL
        ...
        AND rowid < (
          SELECT rowid
          FROM last_persist
          WHERE last_persist.goblin = actions.goblin
        )
      ORDER BY rowid ASC
    `;
    ...
    this._queries.countPersistsFrom = `
      SELECT COUNT(*) as count
      FROM actions
      WHERE type = 'persist';
    `;

    this._queries.countNewPersistsFrom = `
      SELECT COUNT(*) as count
      FROM actions
      WHERE rowid > (
        SELECT MAX(rowid)
  ...
  _piscina(db) {
    if (!this.#piscina[db]) {
      this.#piscina[db] = new Piscina({
        filename: path.join(__dirname, './workers/insert.js'),
        idleTimeout: 1000,
        minThreads: 1,
        maxThreads: 1,
      });
  ...
  get _piscinaSync() {
    if (!this.#piscinaSync) {
      this.#piscinaSync = new Piscina({
        filename: path.join(__dirname, './workers/retrieve.js'),
        idleTimeout: 30000,
        minThreads: 0,
      });
    }
    return this.#piscinaSync;
  }
  ...
  _getIndices(db) {
    ...
    if (enableFTS && this.#userIndices[_db]) {
      const entries = Object.entries(this.#userIndices[_db]);
      for (const [type, _indices] of entries) {
        for (const index of _indices) {
          indices.push(`
            CREATE INDEX IF NOT EXISTS json_${type}_${index}
              ON lastPersistedActions (json_extract(action, '$.payload.state.${index}'), goblin)
              WHERE goblin GLOB '${type}-*';
          `);
        }
      }
    }
      ...
      () => {
        ...
        if (version < 11) {
          if (enableFTS && this.#userIndices[dbName]) {
            const entries = Object.entries(this.#userIndices[dbName]);
        ...
        if (version < 12) {
          super.exec(
            dbName,
            `DROP INDEX IF EXISTS goblin;
             PRAGMA user_version = ${versionPragma};`
          );
        }
        if (version < 13) {
          resp.events.send('host-alert-dialog', {
            cmd: 'open',
            alert: `Database schema migration is running for "${dbName}", please wait...`,
          });

          super.exec(dbName, `DROP TRIGGER IF EXISTS actions_ai;`);

          try {
            super.exec(
              dbName,
              `ALTER TABLE lastPersistedActions DROP COLUMN timestamp;`
            );
          } catch (ex) {
            resp.log.warn(ex.stack || ex.message || ex);
          }
  ...
  _setIndices(db, indices, type = '_default') {
    if (!this.#userIndices[db]) {
      this.#userIndices[db] = {};
    }
    if (!this.#userIndices[db][type]) {
      this.#userIndices[db][type] = [];
    }
    this.#userIndices[db][type].push(...indices);
  }
  ...
  async init(resp, msg) {
    ...
    const {db} = msg.data;
    const res = !!this._open(db, resp);

    if (res && this.#vec.get(db)) {
      this._piscina(db)
        .run(
          {
            db,
          ...
          {name: 'refreshEmbeddings'}
        )
        .catch((ex) => resp.log.err(ex.stack || ex.message || ex));
    }
  ...
  async freeze(resp, msg) {
    ...
    try {
      ...
      await SQLite.wait(() => {
        /* Check if this persist is necessary */
        if (!commitId && action?.type === 'persist') {
          const result = this.stmts(db).getPersist.get({goblin});
          if (result?.action) {
            const state0 = result.action.replace(
              /.*"payload":{.*"state":(.*)},"type":"persist"}$/,
              '$1'
            );
            const state1 = _action.replace(
              /.*"payload":{.*"state":(.*)},"type":"persist"}$/,
              '$1'
            );
            /* Revert staged actions because they are useless */
            if (state0 === state1) {
              this.stmts(db).revert.run({goblin});
  ...
  countPersistsFrom(resp, msg) {
    ...
    return this.stmts(db).countPersistsFrom.get()?.count || 0;
  }

  countNewPersistsFrom(resp, msg) {
    if (!this.tryToUse()) {
      return;
    }
    ...
    return this.stmts(db).countNewPersistsFrom.get({commitId})?.count || 0;
  }
  ...
  getPersistFromRange(resp, msg) {
    ...
    this._piscinaSync.run(
      ...
      {name: 'getPersistFromRange', transferList: [port2]}
    );
  ...
  getAllPersist(resp, msg) {
    ...
    const count = this.stmts(db).countPersistsFrom.get()?.count || 0;
    const {port1, port2} = new MessageChannel();
    this._piscinaSync.run(
      {
        port: port2,
      ...
      {name: 'getAllPersist', transferList: [port2]}
    );

    const stream = new MessagePortReadable(port1);
    return {xcraftStream: stream, routingKey: getRoutingKey(), count};
  }
  ...
  *bootstrapActions(resp, msg, next) {
    ...
    const {db, streamId, routingKey, rename, count} = msg.data;
    const dbPath = this._path(db);
    ...
    this.close(db);
    yield unlinkDatabase(dotdbPath);
    ...
    try {
      stream = new WritableSQL(
        ...
        64,
        (pos) =>
          resp.events.send('cryo.bootstrapActions.progress', {pos, max: count})
      );
  ...
  async revert(resp, msg) {
    ...
    await SQLite.wait(() => this.stmts(db).revert.run({goblin}));
  }
  ...
  async refreshEmbeddings(resp, msg) {
    if (!this.tryToUse()) {
      return;
    }
    ...
    let {dbs} = msg.data;
    if (!dbs || !dbs.length) {
      dbs = this.getAllNames().filter((db) => this.#vec.get(db));
    }
