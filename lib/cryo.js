...
class Cryo extends SQLite {
  ...
  _open(dbName, resp) {
    ...
    this.#soulSweeper[dbName] = new SoulSweeper(this._db[dbName], dbName);
    ...
    for (let wait = true; wait; ) {
      try {
        ...
        wait =
          ...
          ex.message === 'This database connection is busy executing a query';
        if (!wait) {
          throw ex;
        }
        await timeout(400);
  ...
  isEmpty(resp, msg) {
    ...
    return !this.stmts(db).isEmpty.get();
  }
  ...
  async immediate(resp, msg) {
    ...
    await this._syncLock.lock(this._path(db));
    try {
      await this._wait(() => this.stmts(db).immediate.run());
    } catch (ex) {
  ...
  async exclusive(resp, msg) {
    ...
    await this._syncLock.lock(this._path(db));
    try {
      await this._wait(() => this.stmts(db).exclusive.run());
    } catch (ex) {
      this._syncLock.unlock(this._path(db));
      throw ex;
    }
  ...
  async commit(resp, msg) {
    ...
    await this._wait(() => this.stmts(db).commit.run());
  ...
  async freeze(resp, msg) {
    ...
    try {
      ...
      await this._wait(() => this.stmts(db).freeze.run(payload));
  ...
  *branch(resp, msg, next) {
  ...
  getPersistFromRange(resp, msg) {
    ...
    return fromCommitId
      ...
      : this.stmts(db).lastPersistTo.all({toCommitId});
  }
  ...
  getAllPersist(resp, msg) {
    ...
    const stream = new ReadableSQL(this.stmts(db).lastPersist);
  ...
  *bootstrapActions(resp, msg, next) {
    ...
    const {db, streamId, routingKey} = msg.data;
    if (!this._open(db, resp)) {
      return;
    }

    const stream = new WritableSQL(
      this.stmts(db).freeze,
      this.stmts(db).begin,
      this.stmts(db).commit
    );
  ...
  countCreateOrPersist(resp, msg) {
    ...
    if (countCreate?.count && parseInt(countCreate?.count)) {
      return parseInt(countCreate.count);
    }

    const exists = this.stmts(db).hasPersist.get({goblin});
    return exists ? 1 : 0;
  }
  ...
  async prepareDataForSync(resp, msg) {
    ...
    await this._wait(() => stmt.run(commitId, ...rows));
  }
  ...
  async updateActionsAfterSync(resp, msg) {
    ...
    `;
    const stmt = super.prepare(db, commitActionsStmt);
    await this._wait(() => stmt.run(serverCommitId, ...rows));
  }
  ...
  sweep(resp, msg) {
    ...
    for (const db of dbs) {
      ...
      try {
        changes[db] = this.#soulSweeper[db].sweepForDays(30, false);
      } catch (ex) {
