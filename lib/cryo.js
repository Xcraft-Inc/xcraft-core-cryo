...
class Cryo extends SQLite {
  ...
  constructor(cryoConfig, appVersion = null, location = null) {
    ...
    if (enableVEC && enableFTS) {
      ...
      -- Insert and Update are handled by a worker, see this.#worker
  ...
  dispose() {
    for (const unsub of this.#workerUnsub.values()) {
      unsub();
    }
    this.#workerUnsub.clear();
    for (const worker of this.#worker.values()) {
  ...
  _open(dbName, resp, skipIndices = false) {
    if (this._db[dbName]) {
      return true;
    }
      ...
      () => {
        ...
        if (enableVEC) {
          ...
          if (!dbName.startsWith('.')) {
            ...
            const workerVec = require('./sqlite-vec/worker.js');
            const worker = workerVec(defaultLocale, dbName, this.getLocation());
            this.#worker.set(dbName, worker);

            worker
              .on('error', (error) => {
                resp.log.err('Worker error:', error);
              })
              .on('exit', (code) => {
                if (code !== 0) {
                  resp.log.err(`Worker stopped with exit code ${code}`);
                }
              });
            ...
            this.#workerUnsub.set(
              dbName,
              resp.events.subscribe(`*::<worker-vec-embed>`, ({data}) =>
                worker.postMessage({goblin: data})
              )
            );
          }
  ...
  *bootstrapActions(resp, msg, next) {
    ...
    if (!this._open(dotdb, resp, false)) {
      return;
    }
