...
class Cryo extends SQLite {
  ...
  _setNoHistory(db, goblin) {
    if (!this.#noHistory[db]) {
      this.#noHistory[db] = new Set();
    }
    this.#noHistory[db].add(goblin);
  }
  ...
  async freeze(resp, msg) {
    ...
    try {
      ...
      await Cryo.wait(() => {
        ...
        /* Kepp only the last persist (no history) */
        if (
          commitId &&
          payload.type === 'persist' &&
          this.#noHistory[db]?.has(goblin.split('-', 1)[0])
        ) {
