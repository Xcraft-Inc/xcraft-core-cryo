...
class Cryo extends SQLite {
  ...
  constructor(cryoConfig, appVersion = null, location = null) {
    ...
    if (this._cryoConfig?.enableVEC && this._cryoConfig?.enableFTS) {
      this._vecTables = `
      CREATE TRIGGER IF NOT EXISTS lastPersistedActions_ai_vec AFTER INSERT ON lastPersistedActions BEGIN
          INSERT INTO embeddings768 (documentId, chunkId, embedding)
          SELECT 
        ...
        CREATE TRIGGER IF NOT EXISTS lastPersistedActions_ad_vec AFTER DELETE ON lastPersistedActions BEGIN
          DELETE FROM embeddings768
          WHERE documentId = json_extract(NEW.action,'$.payload.state.id');
        END;

        CREATE TRIGGER IF NOT EXISTS lastPersistedActions_au_vec AFTER UPDATE ON lastPersistedActions BEGIN
          DELETE FROM embeddings768
      ...
      () => {
        ...
        if (enableFTS) {
          ...
          if (enableVEC) {
            ...
            this._db[dbName]
              .exec(`CREATE VIRTUAL TABLE IF NOT EXISTS embeddings768 USING vec0(
                documentId TEXT,
                chunkId TEXT,
                embedding FLOAT[768]
            )`);
