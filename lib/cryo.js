    ...
    const res = super.open(
      ...
      () => {
        ...
        if (enableVEC) {
          ...
          //Checking existing data length
          //ex: Dimension de 4096 floats × 4 octets = 16 384 octets
          const exist = super
            .prepare(
              dbName,
              `SELECT length(embedding) as length FROM embeddings;`
            )
            .get();
          if (!exist) {
            resp.log.warn(
              `Should drop embeddings table: unknow dimensions (no-data found)`
            );
            ...
            const existingDimensions = parseInt(exist.length) / 4;
