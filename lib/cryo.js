...
class Cryo extends SQLite {
    ...
    if (
      ...
      if (
        ...
        (!this._cryoConfig?.vec?.list?.length ||
          this._cryoConfig?.vec?.list?.includes(dbName))
      ) {
    ...
    const res = super.open(
      ...
      () => {
        ...
        if (enableVEC) {
          ...
          resp.log.dbg(`Vector extension enabled, migrating...`);
          const dimensions = this._cryoConfig?.vec?.dimensions;
          this._db[dbName]
            .exec(`CREATE VIRTUAL TABLE IF NOT EXISTS embeddings USING vec0(
          ...
          //ex: Dimension de 4096 floats × 4 octets = 16 384 octets
          const blobLength = parseInt(dimensions) * 4;
          const res = this._db[dbName]
            .prepare(
              `
            SELECT EXISTS (
              SELECT 1 FROM embeddings WHERE length(embedding) != ${blobLength}
            ) AS shouldDrop;
          `
            )
            .get();
          if (res.shouldDrop) {
            const res = this._db[dbName]
              .prepare(`SELECT length(embedding) as length FROM embeddings;`)
              .get();
            const existingDimensions = parseInt(res.length) / 4;
            ...
            this._db[dbName].exec(`DROP TABLE embeddings`);
            this._db[dbName].exec(`CREATE VIRTUAL TABLE  embeddings USING vec0(
              locale TEXT partition key,
              scope TEXT,
              documentId TEXT,
              +chunkId TEXT,
          ...
          resp.log.dbg(`Vector extension enabled, migrating...[DONE]`);
        }
      },
      _indices
    );
  ...
  async freeze(resp, msg) {
    ...
    try {
      ...
      await Cryo.wait(() => this.stmts(db).freeze.run(payload));
