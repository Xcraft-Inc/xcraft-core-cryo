...
class Cryo extends SQLite {
  ...
  constructor(cryoConfig, appVersion = null, location = null) {
    ...
    this._queries.getPersistState = `
      SELECT json_extract(action, '$.payload.state') AS state
        FROM actions
       WHERE goblin = $goblin
         AND type = 'persist'
       ORDER BY rowid DESC
    ...
    try {
      ...
      await SQLite.wait(() => {
        ...
        if (!commitId && action?.type === 'persist') {
          const result = this.stmts(db).getPersistState.get({goblin});
          if (result?.state) {
            const state = _action.replace(
