...
class Cryo extends SQLite {
  ...
  _open(dbName, resp) {
    ...
    this._initForWrite(dbName);
    return res;
  }

  _initForWrite(dbName) {
    const enableWorker = this._cryoConfig?.enableWorker;

    if (enableWorker) {
      const Piscina = require('piscina');

      this.#piscina[dbName] = new Piscina({
        filename: path.resolve(__dirname, 'piscina/worker.js'),
        minThreads: 1,
        maxThreads: 1,
        workerData: {
          location: this._cryoDir,
          dbName,
        },
      });
      return;
  ...
  async freeze(resp, msg) {
    ...
    try {
      ...
      await this.#piscina[db].run({payload}, {name: 'freeze'});
