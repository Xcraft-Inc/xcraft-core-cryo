...
class Cryo extends SQLite {
  ...
  _piscinaSync(db) {
    if (!this.#piscinaSync[db]) {
      this.#piscinaSync[db] = new Piscina({
        filename: path.join(__dirname, './workerSync.js'),
        idleTimeout: 1000,
        minThreads: 1,
        maxThreads: 1,
      });
    }
    return this.#piscinaSync[db];
  }
  ...
  dispose() {
    ...
    for (const db of Object.keys(this.#piscinaSync)) {
      this.#piscinaSync[db]
        .close()
        .catch((ex) => console.warn(ex.stack || ex.message || ex));
      delete this.#piscinaSync[db];
    }
  ...
  getPersistFromRange(resp, msg) {
    ...
    const stream = new ReadableSQL(stmt, params, SQLite.wait);
    return {xcraftStream: stream, routingKey: getRoutingKey()};
  }

  async getAllPersist(resp, msg) {
    if (!this.tryToUse()) {
      return;
    }
    ...
    /*const stream = new ReadableSQL(
      this.stmts(db).lastPersist,
      null,
      SQLite.wait
    );*/

    const {port1, port2} = new MessageChannel();
    this._piscinaSync(db).run(
      {
        port: port2,
        db,
        location: this.getLocation(),
